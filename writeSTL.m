function writeSTL(filename, YNtxt, varargin)
%STLWRITE   Write STL file from patch or surface data.
% Call: writeSTL('aa.stl',trig,vtx,'mode','ascii')
...
%       writeSTL('test.stl',fv)         % Save to binary .stl
%
%   Example 2:
%       % Write ascii STL from gridded data
%       [X,Y] = deal(1:40);             % Create grid reference
%       Z = peaks(40);                  % Create grid height
%       writeSTL('test.stl',X,Y,Z,'mode','ascii')
...
if (fid == -1)
    error('writeSTL:cannotWriteFile', 'Unable to write to %s', filename);
end
...
% Close the file
fclose(fid);
if YNtxt=='y' % report file contents
...
if isstruct(varargin{1}) % writeSTL('file', FVstruct, ...)
    ...
    switch numericInputCnt
        case 3 % writeSTL('file', X, Y, Z, ...)
            ...
            elseif numel(varargin{1})==ZsizeXY(1) && numel(varargin{2})==ZsizeXY(2)
                ...
                error('writeSTL:badinput', 'Unable to resolve X and Y variables');
            end
            ...
            if strcmp(options.triangulation,'delaunay')
                ...
                if ~exist('mesh2tri','file')
                    error('writeSTL:missing', '"mesh2tri" is required to convert X,Y,Z matrices to STL. It can be downloaded from:\n%s\n',...
                        'http://www.mathworks.com/matlabcentral/fileexchange/28327')
                end
                [faces, vertices] = mesh2tri(X, Y, Z, options.triangulation);
            end
            
        case 2 % writeSTL('file', FACES, VERTICES, ...)
            faces = varargin{1};
            vertices = varargin{2};
            
        otherwise
            error('writeSTL:badinput', 'Unable to resolve input types.');
    end
...
IP.addParamValue('title', sprintf('Created by writeSTL.m %s',datestr(now)), @ischar);
